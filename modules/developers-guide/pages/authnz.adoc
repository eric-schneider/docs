...
TIP: Both methods can be used simultaneously in https://astra.datastax.com[Astra, DataStax's Database-as-a-Service].
...
TIP: https:///astra.datastax.com[Astra, DataStax's Database-as-a-Service], is backed
...
version: '2'
  ...
  services:
    ...
    stargate-jwt:
      ...
      mem_limit: 2G
      environment: //<6>
        - JAVA_OPTS=-XX:+CrashOnOutOfMemoryError -Xmx750M -Xms64M -Dstargate.auth_id=AuthJwtService -Dstargate.auth.jwt_provider_url=https://keycloak:4444/auth/realms/stargate/protocol/openid-connect/certs
...
[source, bash]
----
USER_TOKEN=$(curl -s --data "username=testuser1&password=testuser1&grant_type=password&client_id=user-service" htts://localhost:4444/auth/realms/stargate/protocol/openid-connect/token | jq -r '.access_token') //<1>
